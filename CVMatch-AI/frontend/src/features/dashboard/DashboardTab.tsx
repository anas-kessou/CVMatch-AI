import { Award, ChevronRight, Clock, FileText, Star, Target, TrendingUp, Users, Zap, Sparkles, Trash2 } from 'lucide-react';
...
const DashboardTab = ({
  ...
  return (
    <div className="space-y-6">
      {/* Banner Mode Démo pour Présentation */}
      <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 rounded-xl p-5 text-white shadow-md border border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="p-3 bg-indigo-500/20 rounded-lg border border-indigo-500/30 text-indigo-400">
            <Sparkles className="w-6 h-6 animate-pulse" />
          </div>
          <div>
            <h4 className="font-bold text-lg text-indigo-200">Mode Démo & Présentation</h4>
            <p className="text-sm text-slate-400">
              {candidates.length > 0
                ? "Données de simulation chargées. Vous pouvez les vider pour présenter l'import en direct ou les recharger."
                : "Aucune donnée n'est actuellement chargée. Remplissez le tableau de bord instantanément pour votre démo."}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3 flex-shrink-0">
          {candidates.length > 0 && (
            <button
              onClick={onClearData}
              className="flex items-center gap-2 px-4 py-2 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/30 rounded-lg text-rose-300 text-sm font-semibold transition-all cursor-pointer whitespace-nowrap"
            >
          ...
          <button
            ...
            className="flex items-center gap-2 px-5 py-2 bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 text-white rounded-lg text-sm font-bold shadow-sm transition-all cursor-pointer border border-indigo-400/20 whitespace-nowrap"
          >
            <Sparkles className="w-4 h-4" />
            {candidates.length > 0 ? "Recharger la démo" : "Charger la démo"}
          </button>
        </div>
      </div>
