      ...
      <div className="px-8 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 px-3 py-1.5 bg-gray-50 rounded-lg border border-gray-100">
            <span className={`inline-block w-2 h-2 rounded-full ${apiStatus === 'online' ? 'bg-emerald-500' : apiStatus === 'offline' ? 'bg-rose-500' : 'bg-amber-500 animate-pulse'}`} />
            <span className="text-xs font-medium text-gray-600">
              {apiStatus === 'online' ? `API: ${apiModel}` : apiStatus === 'offline' ? 'API: Hors-ligne' : 'API: Connexion...'}
            </span>
          </div>
